...
async getTurno(id: number){
        ...
        .subscribe({
              next:async (turno)=>{
               ////////console.log('turno',(turno));
                ...
                //   this.turno.vehiculo.code = this.turno.vehiculo.placa;
                //   this.turno.vehiculo.name = this.turno.vehiculo.placa;
                //   this.turno.vehiculo.label = this.turno.vehiculo.placa;
                ...
                //   this.turno.transportadora.code = this.turno.transportadora.nit;
                //   this.turno.transportadora.name = this.turno.transportadora.nombre;
                //   this.turno.transportadora.label = this.turno.transportadora.nit+' - '+this.turno.transportadora.nombre;
                //   this.transportadoraSeleccionada = this.turno.transportadora
                ...
                //   this.turno.conductor.code = this.turno.conductor.cedula;
                //   this.turno.conductor.name = this.turno.conductor.nombre;
                ...
                //   // let historialTurno$ = this.solicitudTurnoService.getHistorialTurnosByID(turno.id);
                //   // this.infoHistorialTurno = await lastValueFrom(historialTurno$);
                ...
                //    if(turno.estado===EstadosDealleSolicitud.PAUSADO){
                //       historial = this.infoHistorialTurno.detalle_solicitud_turnos_historial.filter((linea: { estado: EstadosDealleSolicitud; }) =>linea.estado === EstadosDealleSolicitud.PAUSADO);
                //       this.messageService.add({severity:'warn', summary: '!Advertencia¡', detail:`El turno se encuentra pausado debido a la siguente novedad: ${JSON.stringify(historial[historial.length-1].novedades.map((novedad: { novedad: any; })=>{return novedad.novedad}).join())}`});
                //   }else{
                //       historial = this.infoHistorialTurno.detalle_solicitud_turnos_historial.filter((linea: { comentario: any; estado: EstadosDealleSolicitud; }) =>linea.estado != EstadosDealleSolicitud.PAUSADO && linea.comentario!=null);
                //       if(historial.length>0){
                //         this.messageService.add({severity:'warn', summary: '!Advertencia¡', detail:` ${this.functionsService.bufferToString(historial[historial.length-1].comentario)}`});
                //       }
                //   }
  ...
  async getHistorialTurno(id:any):Promise<void>{
  ...
  */
  ...
  historialTurno(idTutno?:any){
    ...
    const ref = this.dialogService.open(ListaHistorialTurnoComponent, {
      data: {
          id: (idTutno),
          historial:this.infoHistorialTurno.detalle_solicitud_turnos_historial
      },
   ...
   //////////////console.log('turno para validar estado',this.turno.condiciontpt);
    ...
    let nuevoEstado = "";
            ...
            if(this.updateModulo){
                ...
                // Lotes gestionados de forma independiente — ya persistidos en BD
               ...
               //this.infoHistorialTurno.detalle_solicitud_turnos_historial = turno.detalle_solicitud_turnos_historial;
                ...
                if(this.filesToUpload.length > 0 && this.uploadActivo){
                  for(let anexo of this.filesToUpload){
                    ...
                    body.append('id_relacion', this.infoHistorialTurno.detalle_solicitud_turnos_historial[this.infoHistorialTurno.detalle_solicitud_turnos_historial.length-1].id);
          ...
          if(!error){
              ...
              for(let pedido of pedidosSeleccionados){
                ...
                if(pedidosTurno.find((linea: { pedidonum: any, itemcode:any, municipioentrega:any, lugarentrega:any, linea:any }) => linea.pedidonum == pedido.docnum && 
                  ...
                  let localidad = infoPedido.localidad;

                  let tarifa_tonelada = this.condicion_tpt=='TRANSP' && pedido.itemcode.startsWith('SF')?infoPedido.precio_coniva:0; 
                  let nombre_asistente = infoPedido.nombre_asistente;
                  let email_asistente = infoPedido.email_asistente;
                  let Maneja_lote = infoPedido.Maneja_Lote;
                  let tipo_documento = infoPedido.tipo_pedido;
                  let objectType = infoPedido.ObjType;
                  let vicepresidencia = infoPedido.vicepresidencia;
                  let ivacode = infoPedido.ivacode;
                  let precio_lista = infoPedido.precio_lista;
                  let precio_vendedor = infoPedido.precio_vendedor;
                  let precio_gerente = infoPedido.precio_gerente;
                  let categoria_item = infoPedido.categoria_item;
                  let subcategoria_item = infoPedido.subcategoria_item;
                  let almacen_fpp = infoPedido.almacen_fpp;
                  let bodega_destino = infoPedido.bodega_destino;
                  let bodega_final = infoPedido.bodega_final;
                  let ubicacion = infoPedido.ubicacion;
                  let codigo_vendedor = infoPedido.codigo_vendedor;
                  let precio_unitario = infoPedido.precio_unitario;
                  let tipo_operacion = infoPedido.tipo_operacion;
                  let codigo_ubicacion = infoPedido.codigo_ubicacion
 ...
 async cambioHoraCitaCambioBodega():Promise<void>{
  ...
  buttonLine($event:any){
    ...
    if(campo==="lote_produccion"){
      ...
      // Mapear id_bd desde el id de BD para cada lote ya persistido
      this.lotesItemLine = (this.itemLineSelected[0].detalle_lotes_item_turno || []).map((lote: any) => ({
        ...lote,
        id_bd: lote.id_bd ?? lote.id
      }));
  ...
  borrarLote(){
    ...
    if(loteAEliminar.id_bd){
      this.solicitudTurnoService.eliminarLote(loteAEliminar.id_bd).subscribe({
        next: () => {
        ...
        error: (err) => {
          console.error(err);
          this.messageService.add({severity:'error', summary:'Error', detail:'No se pudo eliminar el lote: ' + err.error?.message});
        }
      });
    } else {
      this.lotesItemLine.splice(index, 1);
  ...
  async asignarLotesItem(){
    ...
    const pedidoId = this.itemLineSelected[0].id;
    ...
    if(parseFloat(totalTon) ===0 || parseFloat(totalSacos)===0 ){
      ...
      // Persistir lotes nuevos (los que no tienen id asignado desde BD)
      const lotesNuevos = this.lotesItemLine.filter(l => !l.id_bd);
      if(lotesNuevos.length > 0){
        const payload = lotesNuevos.map(l => ({
          lote: l.lote,
          cantidad_bodega_lote: l.cantidad_bodega_lote,
          cantidad_cargue_lote: l.cantidad_cargue_lote,
          cantidad_sacos_lote: l.cantidad_sacos_lote,
          fecha_vencimiento: l.fecha_vencimiento
        }));
        const lotesGuardados: any[] = await lastValueFrom(this.solicitudTurnoService.crearLotesPedido(pedidoId, payload));
        // Asignar id_bd a cada lote recién guardado
        lotesNuevos.forEach((l, i) => { l.id_bd = lotesGuardados[i]?.id; });
      }

      // Actualizar lotes existentes que hayan sido modificados
      const lotesExistentes = this.lotesItemLine.filter(l => l.id_bd);
      for(const lote of lotesExistentes){
        await lastValueFrom(this.solicitudTurnoService.actualizarLote(lote.id_bd, {
          ...
          cantidad_bodega_lote: lote.cantidad_bodega_lote
        }));
      }
      ...
      let lotes = this.lotesItemLine.map((linea)=>linea.lote).join(', ');
      ...
      this.pedidosTurno[indexItemTurno].detalle_lotes_item_turno = this.lotesItemLine;
      this.pedidosTurno[indexItemTurno].lineaUpdate.update = true;
      this.formGestionLotesItem = false;

      this.messageService.add({severity:'success', summary:'Confirmación', detail:'Lotes guardados correctamente'});
    }
